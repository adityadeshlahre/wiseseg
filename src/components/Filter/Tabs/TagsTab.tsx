...
const TagsTab: React.FC<Props> = ({
  ...
  const handleTagClick = (tag: { id: number; lable: string; key: string }) => {
    const isAlreadySelected = selectedTagList.some((item) => item.id === tag.id)
    if (isAlreadySelected) {
      setSelectedTagList(selectedTagList.filter((item) => item.id !== tag.id))
      handleRemoveNumberOfFiltersApplied()
    } else {
